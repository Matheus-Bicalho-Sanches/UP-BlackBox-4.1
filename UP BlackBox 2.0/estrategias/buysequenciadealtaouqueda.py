...
def run_buysequenciadealtaouqueda(csv_path, x=3, y=5, stop_loss=-0.05, take_profit=0.08):
    """
    Estratégia:
    - Compra no fechamento após uma sequência de X altas (ou |X| quedas se X<0)
    - Venda após Y períodos, ou se atingir stop loss (Z) ou take profit (W)
    Parâmetros:
      x: número de altas (positivo) ou quedas (negativo) consecutivas para comprar
      y: número de períodos para segurar a posição
    ...
    # Calcular se houve sequência de altas/quedas
    df['retorno'] = df['close'].pct_change()
    if x > 0:
        df['sequencia'] = (df['retorno'] > 0).astype(int)
    else:
        df['sequencia'] = (df['retorno'] < 0).astype(int)
    # Rolling sum para contar sequência
    df['sequencia_acumulada'] = df['sequencia'].rolling(abs(x)).sum()
    sinais = (df['sequencia_acumulada'] == abs(x))
    ...
    while i < len(df):
        if sinais.iloc[i]:
            ...
            saida_idx = min(entrada_idx + y, len(df) - 1)
            saida_data = df.at[saida_idx, 'date']
            saida_preco = df.at[saida_idx, 'close']
            # Checar stop loss/take profit
            for j in range(entrada_idx + 1, saida_idx + 1):
                min_preco = df.at[j, 'low'] if 'low' in df.columns else df.at[j, 'close']
                max_preco = df.at[j, 'high'] if 'high' in df.columns else df.at[j, 'close']
