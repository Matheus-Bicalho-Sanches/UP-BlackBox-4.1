...
def run_operandotoposefundos(csv_path, modo='topo', x=0.10, y=60, w=10, stop_loss=-0.05, take_profit=0.10):
    """
    Estratégia:
    - Compra quando o ativo está X% abaixo do topo (modo='topo') ou X% acima do fundo (modo='fundo') dos últimos Y períodos.
    - Mantém a posição até take profit, stop loss ou W períodos.
    Parâmetros:
      modo: 'topo' ou 'fundo'
      x: percentual (ex: 0.10 para 10%)
      y: janela para buscar topo/fundo (em períodos)
    ...
    for i in range(y, len(df)):
        if modo == 'topo':
            topo = df['close'][i-y:i].max()
            if topo > 0 and (df['close'][i] <= topo * (1 - x)):
                sinais.append(i)
        elif modo == 'fundo':
            fundo = df['close'][i-y:i].min()
            if fundo > 0 and (df['close'][i] >= fundo * (1 + x)):
                sinais.append(i)
    trades = []
