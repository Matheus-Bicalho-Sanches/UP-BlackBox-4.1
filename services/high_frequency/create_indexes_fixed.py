#!/usr/bin/env python3
"""
Script corrigido para criar índices em TimescaleDB
"""
import psycopg
import time
import os
...
def create_performance_indexes():
    """Cria os índices de performance mais importantes para TimescaleDB"""
    ...
    try:
        print('🚀 INICIANDO CRIAÇÃO DE ÍNDICES DE PERFORMANCE (TIMESCALEDB)')
        print('=' * 70)
        
        # Lista de índices para criar (sem CONCURRENTLY para TimescaleDB)
        indexes_to_create = [
            {
        ...
        for idx_info in indexes_to_create:
            ...
            print(f'      📊 Tabela: {idx_info["table"]}')
            print(f'      ⏱️  Tempo estimado: {idx_info["estimated_time"]}')
            
            try:
                start_time = time.time()
                
                # Para TimescaleDB, não usar transação para criação de índices
                if idx_info['table'] == 'ticks_raw':
                    # Para hypertables, usar autocommit
                    conn.autocommit = True
                    cur.execute(idx_info['sql'])
                    conn.autocommit = False
                else:
                    # Para tabelas normais, usar transação
                    cur.execute(idx_info['sql'])
                    conn.commit()
