"""
Debug do Problema de Configuração
================================
Verifica se o problema está na configuração do detector.
"""
...
async def debug_config_issue():
    """Debug do problema de configuração"""
    
    try:
        logger.info("🔍 Debug do Problema de Configuração")
        logger.info("=" * 40)
        ...
        # Mostra configuração do detector TWAP à Mercado
        ...
        logger.info(f"  - time_consistency_threshold: {detector.market_twap_detector.config.time_consistency_threshold}")
        logger.info(f"  - max_interval_minutes: {detector.market_twap_detector.config.max_interval_minutes}")
        ...
        # Testa com configuração MUITO permissiva
        logger.info("🔍 Testando com configuração MUITO permissiva...")
        
        from market_twap_detector import MarketTWAPDetector, MarketTWAPConfig
        permissive_config = MarketTWAPConfig()
        permissive_config.min_volume_repetitions = 2  # Mínimo possível
        permissive_config.min_volume_frequency = 0.1  # 10% apenas
        permissive_config.min_confidence = 0.1  # 10% apenas
        permissive_config.min_time_intervals = 1  # Mínimo possível
        permissive_config.time_consistency_threshold = 0.1  # Muito baixo
        
        permissive_detector = MarketTWAPDetector(permissive_config)
        
        patterns = await permissive_detector.detect_market_twap_patterns(agent_122_trades)
        
        if patterns:
            logger.info(f"✅ Padrões detectados com configuração permissiva: {len(patterns)}")
            for pattern in patterns:
                logger.info(f"  - {pattern.symbol} - Agente {pattern.agent_id} - {pattern.robot_type} - Confiança: {pattern.confidence_score:.2f}")
        else:
            logger.warning("⚠️ Nenhum padrão detectado mesmo com configuração permissiva")
            ...
            if market_trades:
                ...
                for volume, count in volume_counts.most_common():
                    ...
                    if count >= permissive_config.min_volume_repetitions and frequency >= permissive_config.min_volume_frequency:
                        ...
                        if len(volume_trades) > 1:
                            ...
                            if avg_interval <= (permissive_config.max_interval_minutes * 60):
                                logger.info(f"    ✅ Intervalo dentro do limite ({permissive_config.max_interval_minutes} min)")
                            else:
                                logger.info(f"    ❌ Intervalo muito alto ({avg_interval/60:.2f} min > {permissive_config.max_interval_minutes} min)")
                    else:
                        logger.info(f"    ❌ Não atende aos critérios (min_repetitions: {permissive_config.min_volume_repetitions}, min_frequency: {permissive_config.min_volume_frequency})")
...
if __name__ == "__main__":
    asyncio.run(debug_config_issue())
