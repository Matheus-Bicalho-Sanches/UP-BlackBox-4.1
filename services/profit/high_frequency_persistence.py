...
class HighFrequencyPersistence:
        ...
        try:
            with self._pool_lock:
                if self._connection_pool:
                    ...
                    # Cria nova conexão se pool vazio
                    conn = psycopg.connect(self.database_url)
            ...
            if conn:
                try:
                    # Retorna conexão ao pool se estiver saudável
                    if not conn.closed:
                        with self._pool_lock:
    ...
    def _trigger_batch_processing(self, symbol: str):
        ...
        # Cria task assíncrona para processar o lote
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    self.logger.error(f"All retry attempts failed for tick batch")
                    return False
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    # Última tentativa falhou
                    self.logger.error(f"All retry attempts failed for candle batch")
                    return False
    ...
    async def _process_timeout_batches(self):
        ...
        with self._batch_lock:
            # Processa ticks com timeout
            for symbol in list(self.tick_batches.keys()):
                if self.tick_batches[symbol]:
                    asyncio.create_task(self._process_tick_batch(symbol))
            
            # Processa candles com timeout
            for symbol in list(self.candle_batches.keys()):
                if self.candle_batches[symbol]:
                    asyncio.create_task(self._process_candle_batch(symbol))
    ...
    def force_flush(self):
        ...
        with self._batch_lock:
            ...
            # Processa todos os candles pendentes
            for symbol in list(self.candle_batches.keys()):
                if self.candle_batches[symbol]:
                    asyncio.create_task(self._process_candle_batch(symbol))
