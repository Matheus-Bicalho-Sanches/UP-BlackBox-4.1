...
// Interface para representar uma tarefa mensal
interface Task {
  id?: string;
  title: string;
  description: string;
  executionDay: number; // Dia do mês para execução (1-31)
...
// Referência para a coleção de tarefas mensais
const tasksCollection = adminDb.collection('tasks').doc('types').collection('mensal');

// GET - Buscar tarefas mensais
export async function GET(request: Request) {
  try {
    ...
    const snapshot = await query.orderBy('executionDay', 'asc').orderBy('createdAt', 'desc').get();
    ...
    return NextResponse.json(
      { error: 'Erro ao buscar tarefas mensais' },
      { status: 500 }
    );
  }
}

// POST - Criar nova tarefa mensal
export async function POST(request: Request) {
  try {
    ...
    // Validar executionDay
    if (body.executionDay === undefined || typeof body.executionDay !== 'number' || body.executionDay < 1 || body.executionDay > 31) {
      return NextResponse.json(
        { error: 'Dia de execução inválido. Deve ser um número entre 1 e 31.' },
        { status: 400 }
      );
    }
    ...
    console.error('Erro ao criar tarefa mensal:', error);
    return NextResponse.json(
      { error: 'Erro ao criar tarefa mensal' },
      { status: 500 }
    );
  }
}

// PUT - Atualizar tarefa mensal existente
export async function PUT(request: Request) {
  try {
    ...
    // Validar executionDay se fornecido
    if (body.executionDay !== undefined && (typeof body.executionDay !== 'number' || body.executionDay < 1 || body.executionDay > 31)) {
      return NextResponse.json(
        { error: 'Dia de execução inválido. Deve ser um número entre 1 e 31.' },
        { status: 400 }
    ...
    const updateData: Record<string, any> = {
      ...
      executionDay: body.executionDay !== undefined ? body.executionDay : existingTask.executionDay,
    ...
    const taskData: Task = {
      ...
      executionDay: updateData.executionDay ?? existingTask.executionDay ?? 1
    } as Task;
    ...
    console.error('Erro ao atualizar tarefa mensal:', error);
    return NextResponse.json(
      { error: 'Erro ao atualizar tarefa mensal' },
      { status: 500 }
    );
  }
}

// DELETE - Excluir tarefa mensal
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: 'Erro ao excluir tarefa mensal' },
      { status: 500 }
    );
  }
} 