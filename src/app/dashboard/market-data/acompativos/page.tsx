...
import { startOfMinuteBucket, startOfDay, startOfISOWeek } from "@/lib/timezone";
...
export default function AcompativosPage() {
  ...
  const aggregateCandles = (data: any[], tf: string) => {
    ...
    if (tf in tfMap) {
      const size = tfMap[tf];
      data.forEach(c=>{
        const key = size < 60 ? startOfMinuteBucket(c.t, size) : startOfMinuteBucket(c.t, 60); // 60m ainda baseado em minuto 0
      ...
      data.forEach(c=>{
        const key = tf === "1d" ? startOfDay(c.t) : startOfISOWeek(c.t);
        const bucket = map[key] ||= { t:key, o:c.o, h:c.h, l:c.l, c:c.c, v:0, vf:0 };
        bucket.h = Math.max(bucket.h, c.h);
        bucket.l = Math.min(bucket.l, c.l);
        bucket.c = c.c;
  ...
  const handleUnsubscribe = async (ticker: string) => {
    try {
      ...
      setSubscribed((prev) => {
        ...
        // Se o ativo removido estava em foco, escolher o próximo determinístico (alfabético)
        if (tracking === ticker) {
